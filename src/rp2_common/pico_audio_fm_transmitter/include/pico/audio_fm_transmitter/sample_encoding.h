...
#ifndef _PICO_AUDIO_FM_TRANSMITTER_SAMPLE_ENCODING_H
#define _PICO_AUDIO_FM_TRANSMITTER_SAMPLE_ENCODING_H
...
#define program_name fm_transmitter_one_bit_dither
#define NATIVE_BUFFER_FORMAT AUDIO_BUFFER_FORMAT_PIO_FM_TRANSMITTER_CMD1
#else
#define program_name fm_transmitter_two_bit_dither
#define NATIVE_BUFFER_FORMAT AUDIO_BUFFER_FORMAT_PIO_FM_TRANSMITTER_CMD3
#endif
...
typedef struct {
    ...
    uint32_t d, e, f;
#endif
} fm_transmitter_cmd_t; // what we send to PIO for each sample
const fm_transmitter_cmd_t silence_cmd = {SILENCE_CMD, SILENCE_CMD, SILENCE_CMD,
#ifdef AUDIO_HALF_FREQ
...
typedef uint32_t fm_transmitter_cmd_t; // what we send to PIO for each sample
const fm_transmitter_cmd_t silence_cmd = SILENCE_CMD;
#else
typedef struct {
    uint32_t a;
    uint32_t b;
} fm_transmitter_cmd_t; // what we send to PIO for each sample
const fm_transmitter_cmd_t silence_cmd = { SILENCE_CMD, SILENCE_CMD };
#endif
