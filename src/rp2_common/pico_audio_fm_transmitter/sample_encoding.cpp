...
#ifndef PICO_AUDIO_FM_TRANSMITTER_DEFAULT_CORRECTION_MODE
#define PICO_AUDIO_FM_TRANSMITTER_DEFAULT_CORRECTION_MODE dither
#endif

static enum audio_correction_mode audio_correction_mode = PICO_AUDIO_FM_TRANSMITTER_DEFAULT_CORRECTION_MODE;

struct FmtFM_TRANSMITTER  : public FmtDetails<fm_transmitter_cmd_t> {
};

bool audio_fm_transmitter_set_correction_mode(enum audio_correction_mode mode)
{
...
enum audio_correction_mode audio_fm_transmitter_get_correction_mode()
{
    return audio_correction_mode;
}

template <typename FromFmt> void
    __no_inline_not_in_flash_func(encode_samples_none)(int s_count, const typename FromFmt::sample_t *s, fm_transmitter_cmd_t *encoded)
{
...
#if !PIO_AUDIO_FM_TRANSMITTER_NO_INTERP_SAVE
...
#endif
template <typename FromFmt> void
__no_inline_not_in_flash_func(encode_samples_fixed_dither)(int s_count, const typename FromFmt::sample_t *s, fm_transmitter_cmd_t *encoded)
{
    const typename FromFmt::sample_t *s_end = s + s_count * FromFmt::channel_count;
    // hacky cast to allow use to DITHER_BITS > 1
    uint32_t *e = (uint32_t *) encoded;
...
template <typename FromFmt> void
    __no_inline_not_in_flash_func(encode_samples_dither)(int s_count, const typename FromFmt::sample_t *s, fm_transmitter_cmd_t *encoded)
{
...
#if PICO_AUDIO_FM_TRANSMITTER_INTERP_SAVE
    interp_hw_save_t saver;
    interp_save(interp0, &saver);
#endif
...
template <typename FromFmt> void
    __no_inline_not_in_flash_func(encode_samples_noise_shaped_dither)(int s_count, const typename FromFmt::sample_t *s, fm_transmitter_cmd_t *encoded) {
...
// encoding converter
template<typename FromFmt> struct converting_copy<FmtFM_TRANSMITTER,FromFmt> {
    static void copy(typename FmtFM_TRANSMITTER::sample_t *dest, const typename FromFmt::sample_t *src, uint sample_count) {
...
void producer_pool_blocking_give_to_fm_transmitter_s16(audio_connection_t *connection, audio_buffer_t *buffer)
{
    producer_pool_blocking_give<FmtFM_TRANSMITTER, FmtS16>(connection, buffer);
}

void producer_pool_blocking_give_to_fm_transmitter_s8(audio_connection_t *connection, audio_buffer_t *buffer)
{
    producer_pool_blocking_give<FmtFM_TRANSMITTER, FmtS8>(connection, buffer);
}

void producer_pool_blocking_give_to_fm_transmitter_u16(audio_connection_t *connection, audio_buffer_t *buffer)
{
    producer_pool_blocking_give<FmtFM_TRANSMITTER, FmtU16>(connection, buffer);
}

void producer_pool_blocking_give_to_fm_transmitter_u8(audio_connection_t *connection, audio_buffer_t *buffer)
{
    producer_pool_blocking_give<FmtFM_TRANSMITTER, FmtU8>(connection, buffer);
}